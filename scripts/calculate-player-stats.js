...
// 아이템 타입별 주요 스탯
const ITEM_PRIMARY_STATS = {
  helmet: "defense",
  armor: "defense",
  pants: "defense",
  gloves: "additionalAttackChance",
  shoes: "additionalAttackChance",
  shoulder: "additionalAttackChance",
  earring: "defensePenetration",
  ring: "defensePenetration",
  necklace: "defensePenetration",
  mainWeapon: "attack",
  subWeapon: "attack",
  pet: "attack",
  wealthPotion: "creditPerSecondBonus",
  bossPotion: "criticalDamageMultiplier",
  artisanPotion: "criticalChance",
};
...
function calculatePlayerStats(stage) {
  ...
  const equipmentSlots = [
    { type: "helmet", stat: "defense" },
    { type: "armor", stat: "defense" },
    { type: "pants", stat: "defense" },
    { type: "gloves", stat: "additionalAttackChance" },
    { type: "shoes", stat: "additionalAttackChance" },
    { type: "shoulder", stat: "additionalAttackChance" },
    { type: "earring", stat: "defensePenetration" },
    { type: "ring", stat: "defensePenetration" },
    { type: "necklace", stat: "defensePenetration" },
    { type: "mainWeapon", stat: "attack" },
    { type: "subWeapon", stat: "attack" },
    { type: "pet", stat: "attack" },
    { type: "bossPotion", stat: "criticalDamageMultiplier" },
    { type: "artisanPotion", stat: "criticalChance" },
  ];

  equipmentSlots.forEach((slot) => {
    const baseStats = GRADE_BASE_STATS[grade];
    const primaryStat = ITEM_PRIMARY_STATS[slot.type];
    ...
    // 해당 스탯에 추가
    if (stats[primaryStat] !== undefined) {
...
console.log("🎮 스테이지별 플레이어 예상 스탯 분석\n");
...
keyStages.forEach((stage) => {
  ...
  const recommendedBossHP = Math.floor(playerStats.effectiveDPS * targetTurns);
  ...
  // 보스 방어력: 플레이어 공격력의 30-40% 정도 감소시키도록
  const targetDamageReduction = 0.35;
